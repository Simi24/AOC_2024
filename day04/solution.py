        ...
        for c in range(len(matrix[r])):
            if matrix[r][c] == "X" and not visited[r][c]:
                def mark_and_count(start_r, start_c, dr, dc):
                    nonlocal numXmas
                    if (0 <= start_r + 3*dr < len(matrix) and 
                        0 <= start_c + 3*dc < len(matrix[0])):
                        if (matrix[start_r + dr][start_c + dc] == "M" and 
                            matrix[start_r + 2*dr][start_c + 2*dc] == "A" and 
                            matrix[start_r + 3*dr][start_c + 3*dc] == "S"):
    ...
    for r in range(len(matrix)):
        for c in range(len(matrix[r])):
            if matrix[r][c] == "A" and not visited[r][c]:
                if(r + 1 < len(matrix) and c + 1 < len(matrix[0]) and r - 1 >=0 and c-1 >=0):
                    if(((matrix[r - 1][c + 1] == "M" and  matrix[r+1][c-1] == "S") or (matrix[r - 1][c + 1] == "S" and matrix[r+1][c-1] == "M")) and
                       ((matrix[r - 1][c - 1] == "M" and matrix[r+1][c+1] == "S") or (matrix[r - 1][c - 1] == "S" and matrix[r+1][c+1] == "M"))
                       ):
                        visited[r][c] = True
                        numX_Mas += 1
                        print(matrix[r - 1][c + 1], matrix[r+1][c-1], matrix[r - 1][c - 1], matrix[r+1][c+1])
